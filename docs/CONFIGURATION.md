...
In-game saves update the mod's live config immediately. The new values are used the next time a world is loaded or datapacks are reloaded.
...
This mod changes random-spread structure placement. Concentric-ring placement, such as stronghold-style placement, is left unchanged.
